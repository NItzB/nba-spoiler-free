...
export default function BoxScoreModal({ isOpen, onClose, data }: BoxScoreModalProps) {
  ...
  const open = isOpen && !!data && data.length > 0;
  const currentTeamData = data?.[activeTeamIndex];
  const teamInfo = currentTeamData ? getTeam(currentTeamData.team) : null;
  const Logo = teamInfo ? (NBAIcons as any)[teamInfo.abbreviation] : null;

  return (
    <ModalShell isOpen={open} onClose={onClose} panelClassName="max-w-5xl max-h-[90vh] flex flex-col">
      {currentTeamData && teamInfo && (
        <>
          {/* Header */}
          <div className="p-5 sm:p-8 border-b border-white/5 flex items-center justify-between gap-4 bg-gradient-to-b from-white/5 to-transparent">
            <div className="flex items-center gap-5">
              ...
              <div>
                <h2 className="font-display text-2xl sm:text-3xl font-extrabold text-white leading-tight tracking-tight">
